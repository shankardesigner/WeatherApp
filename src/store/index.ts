...
class Store {
  ...
  async loadWeather(city?: string) {
    try {
      const APP_URI: string = `${REACT_APP_BASE_URL}?q=${
        city || REACT_APP_DEFAULT_LOCATION
      }&appid=${REACT_APP_APP_KEY}&cnt=${REACT_APP_DEFAULT_COUNT}`;
      this.loading = true;
      this.error = "";
        ...
        return {
          ...
          temp:
            this.currentTemperature === "C"
              ? this.convertToFahrenheit(dt.temp)
              : this.convertToCelsius(dt.temp),
        };
      });
    });
    //update the current...
    this.setCurrentWeather(moment(date).format("MMM Do"));
